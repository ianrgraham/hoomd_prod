...
parser = argparse.ArgumentParser(description="Quench from infinite temperature to 0 above jamming. Compress until memory hopefully develops.")
...
parser.add_argument('-m','--maxCompression',help="Maximum compression.", type=float, default=1e-1)
parser.add_argument('-y','--compressionStep',help="Step size of compression.", type=float, default=1e-4)
...
parser.add_argument('-p','--phi',help="Packing fraction.", type=float, default=1.1)
...
maxCompression = cmdargs.maxCompression
compressionStep = cmdargs.compressionStep
steps = int(np.rint(maxCompression/compressionStep))
fileSuf = cmdargs.fileSuffix
phi = cmdargs.phi
if fileSuf != "":
    fileSuf += "_"
seeddir = f"/home1/igraham/Projects/hoomd_test/final_data/{fileSuf}initPhi{phi}_N{N}_seed{cmdargs.seed}_maxComp{maxCompression}_compStep{compressionStep}"
...
vlist = [[initial_steps,L]]

Lu=phi_to_L(phi=phi+maxCompression,N=N)
Lb=phi_to_L(phi=phi-maxCompression,N=N)

for i in np.arange(maxIter):
    vlist.extend([[initial_steps+(steps+i*4*steps)*1000, Lu],
        [initial_steps+(3*steps+i*4*steps)*1000, Lb],
        [initial_steps+(4*steps+i*4*steps)*1000, L]])
print(vlist)
...
updater = hoomd.update.box_resize(L = var, period=1000, phase=0)
